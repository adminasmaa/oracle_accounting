...
class InvoiceDiscountCreate extends Component
{
    ...
    public function store()
    {
        ...
        $data = $this->validate([
            ...
            'invoice_discount.discount_quantity' => 'required|numeric',
        ]);

        if ($data['invoice_discount']['discount_quantity'] > 0) {
            InvoiceDiscount::updateOrCreate(['invoice_id' => $data['invoice_discount']['invoice_id']], $data['invoice_discount']);
        }
