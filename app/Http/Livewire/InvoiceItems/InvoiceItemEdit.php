...
class InvoiceItemEdit extends Component
{
    ...
    public function update()
    {
        $data = $this->validate([
            ...
            'invoice_item.purchasing_price' => 'nullable|numeric',
            'invoice_item.selling_price' => 'nullable|numeric',
        ...
        if (!empty($invoice_item_update)) {
            $OldQty = $item->qty;
            $newqty = $invoice_item_update->quantity;
            $oldprice = $item->purchasing_price;
            $newprice = $invoice_item_update->purchasing_price;
            $purchasing_price = ($oldprice * $OldQty + $newprice * $newqty) / ($OldQty + $newqty);
            $piecesqty = $invoice_item_update->unit->pieces * $data['invoice_item']['quantity'];
            $item->update(['purchasing_price' => $purchasing_price, 'qty' => $piecesqty]);
        }
