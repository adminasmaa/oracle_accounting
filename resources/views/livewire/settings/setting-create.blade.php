<div class="d-inline">
    <button type="button" class="btn btn-primary mb-3" data-bs-toggle="modal" data-bs-target="#modalFormSetting">
        انشاء اعداد
    </button>

    <div wire:ignore.self class="modal fade" id="modalFormSetting" tabindex="-1" role="dialog"
                    ...
                    <nav>
                        <div class="nav nav-tabs" id="nav-tab" role="tablist">
                            ...
                            <button class="nav-link" id="nav-profile-tab" data-bs-toggle="tab"
                                    ...
                                    aria-selected="false"> ثوابت النظام
                            </button>
                            <button class="nav-link" id="nav-discount-tab" data-bs-toggle="tab"
                                    ...
                                    aria-selected="false">باقي البيانات
                            </button>
                        </div>
                    </nav>
                    ...
                    <div class="tab-content" id="nav-tabContent">
                        <div class="tab-pane fade show active" id="nav-home" role="tabpanel"
                            ...
                            <div class="form-group">
                                ...
                                @error('setting.company_name')<span
                            ...
                            <div class="form-group">
                                ...
                                @error('setting.company_phone')<span
                                        class="text-danger error">{{ $message }}</span>@enderror
                            </div>
                            ...
                            <div class="form-group col-12 mt-3">
                                ...
                                <select wire:model.defer="setting.payment_selling_account_index_id"
                                        name="index_account_id" class="form-select">
                            ...
                            <div class="form-group col-12 mt-3">
                                ...
                                <select wire:model.defer="setting.payment_parchasing_account_index_id"
                                        name="index_account_id" class="form-select">
                                    <option value="">{{ __('Nothing') }}</option>
                                    @foreach($index_accounts as $index_account)
                                        <option value="{{ $index_account->id }}">{{ $index_account->account_name }}</option>
                        ...
                        <div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
                            ...
                            <div class="form-group col-12 mt-3">
                                ...
                                <select wire:model.defer="setting.inbox_account_index_id" name="index_account_id"
                                        class="form-select">
                                    <option value="">{{ __('Nothing') }}</option>
                                    @foreach($index_accounts as $index_account)
                                        <option value="{{ $index_account->id }}">{{ $index_account->account_name }}</option>
                                ...
                                @error('setting.inbox_account_index_id') <span
                                        class="text-danger error">{{ $message }}</span>@enderror
                            </div>
                            ...
                            <div class="form-group col-12 mt-3">
                                ...
                                <select wire:model.defer="setting.salary_account_index_id" name="index_account_id"
                                        class="form-select">
                                    <option value="">{{ __('Nothing') }}</option>
                                    @foreach($index_accounts as $index_account)
                                        <option value="{{ $index_account->id }}">{{ $index_account->account_name }}</option>
                                ...
                                @error('setting.salary_account_index_id') <span
                                        class="text-danger error">{{ $message }}</span>@enderror
                            </div>
                            <div class="form-group col-12 mt-3">
                                <label for="">حساب الزبائن</label>
                                <select wire:model.defer="setting.customers_account_index_id" name="index_account_id"
                                        class="form-control">
                                    <option value="">{{ __('Nothing') }}</option>
                                    @foreach($index_accounts as $index_account)
                                        <option value="{{ $index_account->id }}">{{ $index_account->account_name }}</option>
                                ...
                                @error('setting.customers_account_index_id') <span
                                        class="text-danger error">{{ $message }}</span>@enderror
                            </div>

                            <div class="form-group col-12 mt-3">
                                <label for="">الخصم المكتسب </label>
                                <select wire:model.defer="setting.discount_earned_account_index_id"
                                        name="index_account_id" class="form-control">
                                    <option value="">{{ __('Nothing') }}</option>
                                    @foreach($index_accounts as $index_account)
                                        <option value="{{ $index_account->id }}">{{ $index_account->account_name }}</option>
                                    @endforeach
                        ...
                        <div class="tab-pane fade" id="nav-discount" role="tabpanel" aria-labelledby="nav-discount-tab">
                            ...
                            <div class="form-group col-12 mt-3">
                                ...
                                <select wire:model.defer="setting.allowed_discount_account_index_id"
                                        name="index_account_id" class="form-control">
                                    <option value="">{{ __('Nothing') }}</option>
                                    @foreach($index_accounts as $index_account)
                                        <option value="{{ $index_account->id }}">{{ $index_account->account_name }}</option>
