<div class="d-inline">
    <button type="button" class="btn btn-sm text-primary border-end" data-bs-toggle="modal"
            data-bs-target=".modalFormEditInvoiceItem{{$invoice_item['id']}}">
        <i class="fa fa-edit"></i>
    </button>

    <div wire:ignore.self class="modal fade modalFormEditInvoiceItem{{$invoice_item['id']}}" tabindex="-1" role="dialog"
                ...
                <form wire:submit.prevent="update" class="modal-body">
                    <div class="row">
                        <div class="form-group col-3">
                            ...
                            <select wire:model.defer="invoice_item.item_id" name="item_id" class="form-select">
                                <option value="">{{ __('Nothing') }}</option>
                                @foreach($items as $item)
                                    <option value="{{ $item->id }}">{{ $item->name }}</option>
                                @endforeach
                            </select>
                            @error('invoice_item.item_id') <span
                                    class="text-danger error">{{ $message }}</span>@enderror
                        </div>
                        <div class="form-group col-2">
                            <label for="">الكمية</label>
                            <input wire:model.defer="invoice_item.quantity" type="number" class="form-control">
                            @error('invoice_item.quantity')<span
                                    class="text-danger error">{{ $message }}</span>@enderror
                        </div>
                        @if($invoice->type == 1)
                            <div class="form-group col-2">
                                <label for="">سعر الشراء</label>
                                <input wire:model.defer="invoice_item.purchasing_price" type="number"
                                       class="form-control">
                            ...
                            <div class="form-group col-3">
                                ...
                                <select wire:model.defer="invoice_item.unit_id" name="unit_id" class="form-select">
                                    <option value="">{{ __('Nothing') }}</option>
                                    @foreach($units as $item)
                                        <option value="{{ $item->id }}">{{ $item->name }}</option>
                                    @endforeach
                                </select>
                                @error('invoice_item.unit_id') <span
                                        class="text-danger error">{{ $message }}</span>@enderror
                            </div>
                        @else
                            <div class="form-group col-2">
