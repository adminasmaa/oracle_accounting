<div class="d-inline">
    <button type="button" class="btn btn-sm text-primary border-end" data-bs-toggle="modal"
            data-bs-target=".modalFormEditArrestReceipt{{$arrest_receipt['id']}}">
        <i class="fa fa-edit"></i>
    </button>

    <div wire:ignore.self class="modal fade modalFormEditArrestReceipt{{$arrest_receipt['id']}}" tabindex="-1"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content rounded-4 px-2">
                ...
                <h5 class="text-primary border-bottom pb-3">ايصالات القبض</h5>

                <form wire:submit.prevent="update" class="modal-body row g-2">

                    <div class="form-group col-6">
                        <label for="">المستخدم</label>
                        <select wire:model.defer="arrest_receipt.user_id" name="user_id" class="form-select">
                    ...
                    <div class="form-group col-6">
                        ...
                        @error('arrest_receipt.batch_quantity')<span
                                class="text-danger error">{{ $message }}</span>@enderror
                    </div>
                    <div class="form-group col-6">
                        <label for="">الوصف</label>
                        <input wire:model.defer="arrest_receipt.description" type="text" class="form-control">
                        @error('arrest_receipt.description')<span
                                class="text-danger error">{{ $message }}</span>@enderror
                    </div>
                    <div class="form-group col-6">
                        <label for="">النوع</label>
                        <select wire:model="arrest_receipt.type" name="type" class="form-select">
