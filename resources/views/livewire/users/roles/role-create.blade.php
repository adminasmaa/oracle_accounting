<div class="d-inline">
    <button type="button" class="btn btn-primary mb-3" data-bs-toggle="modal" data-bs-target="#modalFormrRole">
        {{ __('Create role') }}
    </button>

    <div wire:ignore.self class="modal fade" id="modalFormrRole" tabindex="-1" role="dialog"
                                ...
                                @foreach($permissions as $permission)
                                    <div class="col-md-6">
                                        <label><input type="checkbox" value="{{$permission->id}}"
                                                      wire:model.defer="user_permissions.{{$permission->id}}"> {{$permission->name}}
                                        </label>
                                    </div>
                                @endforeach
                            @else
