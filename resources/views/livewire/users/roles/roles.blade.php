    ...
    @if($roles->count())
        <div class="row">
            <table class="table table-striped table-responsive-sm border">
                <thead>
                <tr>
                    <th scope="col" class="text-center"># 19 19 19</th>
                ...
                @foreach($roles as $role)
                    <tr>
                        ...
                        <td class="text-center ">{{ $role->users ? $role->users()->count() : '0' }}</td>
                        <td class="text-center ">
                            <livewire:users.roles.role-edit :role_id="$role->id"
                                                            :key="'role-edit-roles-'.$role->id"></livewire:users.roles.role-edit>
                            <livewire:users.roles.role-delete :role_id="$role->id"
                                                              :key="'role-delete-roles-'.$role->id"></livewire:users.roles.role-delete>
                        </td>
                    </tr>
                @endforeach
                </tbody>
